...
OPENOCD = openocd
RTOS_AUTO_FILE = gdbrules/rtos_auto.cfg
RTOS_GDB_FLAGS = -ex "target remote | $(OPENOCD) -c \"gdb_port pipe\" -f interface/$(OCD_INTERFFACE) -f target/$(OCD_TARGET).cfg -f $(RTOS_AUTO_FILE)"

GDB_FLAGS = -ex "target remote | $(OPENOCD) -c \"gdb_port pipe\" -f interface/$(OCD_INTERFFACE) -f target/$(OCD_TARGET).cfg"
...
rtos: build/$(PROJECT).elf
	$(GDB) build/$(PROJECT).elf $(RTOS_GDB_FLAGS)
